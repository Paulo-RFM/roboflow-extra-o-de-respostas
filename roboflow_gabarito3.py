...
project = rf.workspace("srinakharinwirot-university-ayja3").project("omr-detect2_v2")

#verificação do download do dataset
if not os.path.exists("OMR-Detect3"):
...
image_dir1 = "OMR-Detect2-3/test/images"
image_dir2 = "OMR-Detect2-3/train/images"
image_dir3 = "OMR-Detect2-3/valid/images"
...
for filename in os.listdir(image_dir1):
    ...
    #print(img)
    filenamefinal = "dataset_result/OMR3/predictTest/"+filename
    modelo.predict(image_path=image_path, confidence=40, overlap=30,labels=false).save(filenamefinal)

  
    pathFinal = 'respostasFinais/OMR3/Test/'+filename
...
for filename in os.listdir(image_dir2):
    ...
    #img = modelo.predict(image_path, confidence=40, overlap=30)
    #print(img)
    filenamefinal = "dataset_result/OMR3/predictTrain/"+filename
    modelo.predict(image_path=image_path, confidence=40, overlap=30,labels=false).save(filenamefinal)

    pathFinal = 'respostasFinais/OMR2/Train/'+filename
    modelo_scan.predict(filenamefinal,confidence=40, overlap=30).save(pathFinal)
...
for filename in os.listdir(image_dir3):
    ...
    #img = modelo.predict(image_path, confidence=40, overlap=30)
    #print(img)
    filenamefinal = "dataset_result/OMR2/predictValid/"+filename
    modelo.predict(image_path=image_path, confidence=40, overlap=30,labels=false).save(filenamefinal)


    pathFinal = 'respostasFinais/OMR2/Valid/'+filename
    modelo_scan.predict(filenamefinal,confidence=40, overlap=30).save(pathFinal)
